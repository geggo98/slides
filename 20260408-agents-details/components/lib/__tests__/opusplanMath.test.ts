...
describe("Defaults bei 1-h-TTL (Write 2×, jedes Claude-Abo im Kontingent)", () => {
  ...
  it("Rückkehr: Brüche 2,344 $ (≈2,05 €) + Re-Plan = 3,469 $ · Ersparnis weg ab 5×", () => {
  ...
  it("Euro-Umrechnung wie paretoData: 9,93 / 24,83 / 15,57 / 21,64 €", () => {
    ...
    expect(toEur(e.antiPattern)).toBeCloseTo(21.64, 2);
  });
  it("Anti-Pattern überholt Nur Opus erst ab der vierten Rückkehr", () => {
    // Mit Sonnet 4.6 ($3/$15) war das schon bei den Default-2 der Fall. Mit
    // Sonnet 5 spart opusplan mehr, also braucht es mehr Rückkehren — die
    // Folie zeigt bei n=2 jetzt einen Anti-Pattern-Balken UNTER „Nur Opus“.
...
describe("Option 5-min-TTL (Write 1,25× — API-Key, Credits, Cloud, Kontingent leer)", () => {
  ...
  it("Szenarien: 10,80 / 27,00 / 16,17 / 21,35 $ (9,46 / 23,65 / 14,16 / 18,70 €)", () => {
    ...
    expect(toEur(e.antiPattern)).toBeCloseTo(18.7, 2);
  });
  it("Brüche ×1/1,6: Bruch 0,419 $ · Break-even 1,12 MTok · Ersparnis weg ab 8×", () => {
