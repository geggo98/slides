 ...
 * Faktor 2,5 · Exec-Read 30 MTok · 3 Re-Plans · Cache-Bruch an):
 *
...
import { BREAK_SHARE, PLAN, READ_FAKTOR } from "../opusplanMath";
...
describe("Cache-Bruch eines Effort-Wechsels", () => {
  it("Sol: 0,93 × 180k × 1,25 × $4 = 0,837 $ (vor Fähigkeitsfaktor)", () => {
    expect(bruchKosten(SOL, CTX)).toBeCloseTo(0.837, 6);
  });
});
...
describe("Invarianten", () => {
  ...
  it("Break-even hängt nicht vom Exec-Volumen ab und bleibt bei Regler-Defaults unter 3,5 MTok", () => {
    for (const m of MODELLE) {
