...
const projVer = computed(() =>
  mode.value === "force"
    ? "+ force(jackson:2.20.2)"
    : "+ strictly(jackson:2.20.2)",
);
...
const explain = computed(() =>
  mode.value === "force"
    ...
    : "strictly() erzwingt 2.20.2 — bei inkompatiblem BOM-Constraint schlägt der Build mit klarem Fehler fehl.",
);
...
<template>
  <div class="scene-wrap">
    <div class="scene-desc">
      <code>force()</code> und <code>strictly()</code> sind die höchsten
      Prioritäten. <code>force()</code> ist still,
      <code>strictly()</code> bricht bei Konflikten ab.
    </div>
    <div class="ctrl-row">
      <label>Modus:</label>
    ...
    <div class="graph">
      ...
      <div class="g-row">
        <GraphNode name="BOM" version="jackson = 2.20.1" variant="loser" />
        <div class="g-gap" />
    ...
    <ResultBox value="jackson:2.20.2" :explain="explain" />
  </div>
</template>
