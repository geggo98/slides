...
// Wiederverwendbare Kreuztabelle (Pivot) mit Klick/Hover-Detail-Blasen.
// Daten als Prop:
//   data = { dimensions:[{key,label,short?}],
//            tools:[{key,name,color?,cells:{<dim>:{teaser,detail,mark}}}] }
//   dimensions = Zeilen, tools = Spalten.
// mark referenziert einen Eintrag aus `markScheme` (Symbol + Farbe je Theme).
const props = defineProps({
  data: { type: Object, required: true },
  // Legende: [{ mark, label }] — Symbol/Farbe stammen aus markScheme[mark].
  legend: { type: Array, default: () => [] },
  // Überschreibt/ergänzt Symbol & Farbe je mark. light/dark = Hex je Theme.
  markScheme: { type: Object, default: () => ({}) },
  hint: { type: String, default: "ⓘ = Klick (oder Hover) für Details" },
});

const dims = computed(() => props.data.dimensions ?? []);
...
const shownDetail = computed(() => {
  const s = shown.value;
  if (!s) return null;
  const tool = tools.value.find((t) => t.key === s.toolKey);
  const dim = dims.value.find((d) => d.key === s.dimKey);
...
// Default-Schema (deckt das ursprüngliche yes/no/partial/none ab); per
// markScheme-Prop überschreib- und erweiterbar.
const DEFAULT_SCHEME = {
  yes: { symbol: "✓", light: "#27500A", dark: "#80c050" },
  no: { symbol: "✗", light: "#A32D2D", dark: "#f06060" },
  partial: { symbol: "◐", light: "#BA7517", dark: "#e0a030" },
  none: { symbol: "", light: "transparent", dark: "transparent" },
};
const scheme = computed(() => ({ ...DEFAULT_SCHEME, ...props.markScheme }));

function markSymbol(m) {
  return scheme.value[m]?.symbol ?? "";
}
function markColor(m) {
  const s = scheme.value[m];
  if (!s) return "transparent";
  return isDark.value ? s.dark : s.light;
}
...
<template>
  <div ref="rootEl" class="pivot">
    <table class="pv">
      ...
      <tbody>
        <tr v-for="d in dims" :key="d.key">
          ...
          >
            <span
              v-if="t.cells[d.key] && markSymbol(t.cells[d.key].mark)"
              class="mark"
              :style="{ color: markColor(t.cells[d.key].mark) }"
              >{{ markSymbol(t.cells[d.key].mark) }}</span
            ><span class="teaser">{{
    ...
    <div class="legend">
      <span v-for="l in legend" :key="l.mark"
        ><span class="lg" :style="{ color: markColor(l.mark) }">{{
          markSymbol(l.mark)
        }}</span>
        {{ l.label }}</span
      >
