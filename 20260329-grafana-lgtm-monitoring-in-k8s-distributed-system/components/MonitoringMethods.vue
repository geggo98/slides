...
const redSignals = computed(() => [
  {
    ...
    detail:
      "rate() in PromQL = (counter_now \u2212 counter_prev) / interval. Counter z\u00E4hlt nur hoch, rate() macht daraus eine Geschwindigkeit.",
    accent: C.value.blue,
  ...
  },
  {
    ...
    detail:
      "Perspektive User: \u201EHat mein Request funktioniert?\u201C Nicht zu verwechseln mit USE-Errors (Infrastruktur-Fehler wie ECC, Packet-Drops).",
    accent: C.value.red,
  ...
  },
  {
    ...
    detail:
      "Kausalit\u00E4t: Saturation \u2192 Duration\u2191 (Warteschlangen). Aber: Duration\u2191 \u21CF immer Saturation. Auch Netzwerk, langsame DB-Queries, GC-Pausen.",
    accent: C.value.orange,
...
const useSignals = computed(() => [
  {
    ...
    detail:
      "Gemessen als Durchschnitt \u00FCber ein Intervall. Achtung: 60% Durchschnitt kann Spitzen von 100% verstecken.",
    accent: C.value.orange,
    color: C.value.orangeDim,
  },
  {
    ...
    detail:
      "Warteschlangentheorie (M/M/1): Ab ~70\u201380% Utilization steigt Wartezeit \u00FCberproportional. Bei 90%: Response-Time = 10\u00D7 Service-Time.",
    accent: C.value.yellow,
    color: C.value.yellowDim,
  },
  {
    letter: "E",
    name: "Errors",
    ...
    detail:
      "Perspektive Maschine: \u201EIst die Hardware fehlerfrei?\u201C Nicht zu verwechseln mit RED-Errors (User-facing HTTP 5xx).",
    accent: C.value.red,
    color: C.value.redDim,
  },
]);
...
const goldenSignals = computed(() => [
  {
    letter: "L",
    name: "Latency",
    desc: "= RED Duration. Erfolgreiche und fehlerhafte Requests getrennt messen.",
    accent: C.value.orange,
    color: C.value.orangeDim,
  },
  {
    letter: "T",
    name: "Traffic",
    desc: "= RED Rate. HTTP Requests/s, aber auch Domain-spezifisch: Quotes/s, Policen/s.",
    accent: C.value.blue,
    color: C.value.blueDim,
  },
  {
    letter: "E",
    name: "Errors",
    desc: "= RED Errors. Explizite (5xx), implizite (200er mit falschem Inhalt), Policy-basiert (>1s = Fehler).",
    accent: C.value.red,
    color: C.value.redDim,
  },
  {
    letter: "S",
    name: "Saturation",
    desc: "Das Extra gegen\u00FCber RED. Misst, wie voll das System ist. Bester Fr\u00FChindikator vor User-Impact.",
    accent: C.value.purple,
    color: C.value.purpleDim,
  },
]);
...
const thresholds = [
  {
    ...
    warn: "70\u201380%",
    crit: "90%+",
    note: "CFS-Throttling beachten, nicht nur Utilization",
  },
  {
    res: "Memory (Container)",
    warn: "80%",
    crit: "90%+",
    note: "container_memory_working_set_bytes, nicht usage_bytes",
  },
  {
    res: "HikariCP Pool",
    warn: "80%",
    crit: "95%+",
    note: "Pending > 0 ist bereits Saturation",
  },
  {
    res: "Tomcat Threads",
    warn: "75%",
    crit: "90%+",
    note: "Gepaart mit Upstream-Timeout-Check",
  },
  {
    res: "Redis Memory",
    warn: "80%",
    crit: "95%+",
    note: "Evictions = aktive S\u00E4ttigung",
  },
  {
    res: "Disk I/O",
    warn: "70%",
    crit: "85%+",
    note: "Queue-Depth > 1 = S\u00E4ttigung beginnt",
  },
];
...
const xyCharts = computed(() => [
  {
    ...
    insight:
      "Zeigt den Knickpunkt, ab dem CPU die Latenz \u00FCberproportional treibt. Hysterese sichtbar wenn Latenz nach Last-Reduktion nicht auf den gleichen Pfad zur\u00FCckkehrt.",
  ...
  },
  {
    ...
    insight:
      "Ab welcher Rate beginnen Errors? Gehen sie bei niedrigerer Rate auf Null zur\u00FCck, oder bleiben sie (Hysterese)?",
    color: C.value.red,
  },
  {
    ...
    insight:
      "Cache-Miss-Amplification: Hit-Ratio sinkt \u2192 Upstream-Rate steigt \u2192 Rate-Limiting \u2192 Feedback-Loop.",
  ...
  >
    ...
    >
      ...
      <div :style="{ marginBottom: '14px' }">
        ...
        >
          Drei komplement&auml;re Methoden, ihre Zusammenh&auml;nge, und die
          Monitoring-Praxis: Schwellwerte, Hysterese-Alerting und
          Phasenraum-Diagnose.
        </p>
      </div>
      ...
      <div v-if="activeTab === 'overview'" class="tab-content">
        ...
        >
          ...
          >
            ...
            >
            <span :style="{ fontSize: '14px', color: C.dim, fontWeight: 300 }"
              >=</span
            >
            <span
              :style="{
          ...
          >
            ...
            >
              <span
                ...
                >{{ m.gs }}</span
              >
              <span :style="{ fontSize: '7px', color: C.dim }"
                >&updownarrow;</span
              >
              <span
                :style="{
        ...
        >
          ...
          >
            ...
            >
              ...
              <div
                :style="{ fontSize: '7.7px', color: C.muted, lineHeight: 1.5 }"
              >
                RED beantwortet: &lsquo;Sind meine User happy?&rsquo; Kein Blick
                auf Infrastruktur. Wenn alle RED-Signale gut sind, aber CPU bei
                95% liegt, sieht RED das nicht &mdash; daf&uuml;r braucht man
                USE.
              </div>
            </div>
          </MethodBox>
          ...
          >
            ...
            >
              ...
              <div
                :style="{ fontSize: '7.7px', color: C.muted, lineHeight: 1.5 }"
              >
                USE beantwortet: &bdquo;Ist die Infrastruktur der
                Engpass?&ldquo; Kein Blick auf User-Experience. Wenn CPU bei 20%
                liegt aber Error-Rate bei 30%, sieht USE kein Problem &mdash;
                daf&uuml;r braucht man RED.
              </div>
            </div>
          </MethodBox>
          ...
          >
            ...
            >
              ...
              <div
                :style="{ fontSize: '7.7px', color: C.muted, lineHeight: 1.5 }"
              >
                F&uuml;r die kritischen Pfade, wo Saturation als
                Fr&uuml;hwarnung den Unterschied macht: Quote-Vergleich,
                Policen-Abschluss, Zahlungs-Flow. RED f&uuml;r alle Services
                einheitlich, Golden Signals zus&auml;tzlich f&uuml;r die
                gesch&auml;ftskritischen.
              </div>
            </div>
          </MethodBox>
        </div>
      ...
      <div v-if="activeTab === 'praxis'" class="tab-content">
        ...
        >
          ...
          >
            Faustregel aus der Warteschlangentheorie (M/M/1): ab ~80%
            Utilization steigt die Wartezeit &uuml;berproportional. Bei 80%: 5x
            Service-Time, bei 90%: 10x. Details und interaktive Simulation im
            Systemdynamik-Simulator.
          </div>
          <div
            :style="{
              display: 'grid',
          ...
          >
            ...
            >
              ...
              <div
                :style="{ fontSize: '7px', color: C.muted, lineHeight: 1.4 }"
              >
                {{ r.note }}
              </div>
            </div>
        ...
        >
          ...
          >
            Systeme kehren nach &Uuml;berlast nicht sofort in den Normalzustand
            zur&uuml;ck (Hysterese): leere Caches, volle Queues, fragmentierte
            JVM-Heaps brauchen Zeit zum Recovery. Naive Schwellwert-Alerts
            ignorieren das und erzeugen Flapping. Grafana hat seit Version 10.3
            ein natives Feature daf&uuml;r.
          </div>
          <div
            :style="{
              display: 'grid',
          ...
          >
            ...
            >
              ...
              >
                Naiver Alert (nur Schwelle)
              </div>
              <div
                :style="{ fontSize: '7.7px', color: C.muted, lineHeight: 1.5 }"
              >
                &ldquo;P99 Latenz &gt; 500ms&rdquo; feuert beim Hinaufgehen und
                resolved beim Heruntergehen. Ignoriert, dass das System bei
                500ms auf dem R&uuml;ckweg noch degradiert ist. Erzeugt Flapping
                wenn die Metrik um die Schwelle pendelt.
              </div>
            </div>
            <div
              :style="{
            ...
            >
              ...
              >
                Mit Recovery Threshold
              </div>
              <div
                :style="{ fontSize: '7.7px', color: C.muted, lineHeight: 1.5 }"
              >
                Feuert bei P99 &gt; 500ms, resolved erst bei P99 &lt; 200ms.
                Asymmetrische Schwellen verhindern Flapping und stellen sicher,
                dass das System wirklich erholt ist bevor der Alert resolved.
              </div>
            </div>
          </div>
          <div
          ...
          >
            <div :style="{ padding: '8px 11px', background: `${C.green}06` }">
              ...
              <div
                :style="{ fontSize: '8.4px', color: C.text, lineHeight: 1.6 }"
              >
                Alert Rule Editor &rarr; Threshold Expression &rarr; Toggle
                &ldquo;Custom recovery threshold&rdquo; &rarr; Wert eingeben.
                Der Alert wechselt in den Zustand &ldquo;Recovering&rdquo; und
                resolved erst, wenn der Recovery-Wert unterschritten wird.
                Zus&auml;tzlich: &ldquo;Keep firing for&rdquo; h&auml;lt den
                Alert f&uuml;r eine konfigurierbare Dauer aktiv nach
                Unterschreitung.
              </div>
            </div>
            <pre
        ...
        >
          ...
          >
            ...
            <strong :style="{ color: C.text }">gegeneinander</strong> (X-Y). Die
            Trajektorie zeigt die Hysterese-Schleife: der Hinweg (Last steigt)
            folgt einem anderen Pfad als der R&uuml;ckweg (Last sinkt). Nutze
            das <strong :style="{ color: C.text }">XY Chart Panel</strong> (ab
            Grafana 10+) f&uuml;r Post-Incident-Reviews.
          </div>
          <div
          ...
          >
            ...
            >
              <div
                :style="{ padding: '7px 10px', background: `${item.color}06` }"
              >
                <div
                  :style="{
                    fontSize: '8.4px',
          ...
          >
            ...
            <div :style="{ fontSize: '7.7px', color: C.text, lineHeight: 1.5 }">
              XY Charts sind prim&auml;r ein Diagnose-Tool f&uuml;r
              Post-Incident-Reviews. Zeitbereich auf die Incident-Dauer setzen,
              Trajektorie analysieren. Die Hysterese-Fl&auml;che quantifiziert
              den Recovery-Verlust. Kombination mit Annotations (Deployment,
              Restart) zeigt, welche Interventionen die Schleife brechen.
            </div>
          </div>
        </div>
      </div>
