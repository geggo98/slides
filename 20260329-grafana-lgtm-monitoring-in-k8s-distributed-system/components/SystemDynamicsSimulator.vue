...
function mm1ToX(u) {
  return mm1Pad.l + (u * mm1Cw) / 0.98;
}
function mm1ToY(f) {
  return mm1Pad.t + mm1Ch - (f / mm1MaxY) * mm1Ch;
}
...
const mm1PathD = computed(() =>
  mm1Pts.value
    .map(
      (p, i) =>
        `${i === 0 ? "M" : "L"}${mm1ToX(p.u).toFixed(1)},${mm1ToY(p.f).toFixed(1)}`,
    )
    .join(" "),
);
...
const caveats = computed(() => [
  ...
  {
    t: "CFS-Throttling",
    d: "Binär, nicht graduell. 80% CPU kann plötzlich 40% Throttling bedeuten bei Bursts.",
    c: C.value.red,
  },
  {
        ...
        <div class="charts-grid">
          ...
          <svg viewBox="0 0 480 58" class="chart-svg">
            ...
            <path
              ...
              :d="
                (() => {
                  const fullSeries = sim.stages.map(
                    (_, i) => result.buffers[i],
                  );
                  const maxVal = computeMaxVal(fullSeries, 0, false);
        ...
        <div class="mm1-panel">
          ...
          <svg :viewBox="`0 0 ${mm1W} ${mm1H}`" class="mm1-svg">
            ...
            <rect
              ...
              @mousemove="mm1MouseMove"
              @mouseleave="mm1MouseLeave"
            />
          </svg>
          <div class="mm1-caption">
            Response-Time = Service-Time / (1 - Utilization). Bei 50%: 2x, bei
            80%: 5x, bei 90%: 10x. Hover für Werte.
          </div>
        </div>
