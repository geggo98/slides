...
const HYSTERESES = [
  {
    ...
    mechanism: 'Unter Überlast werden Cache-Entries evicted oder expiren. Wenn Last sinkt, ist der Cache kalt: jeder Request triggert einen Upstream-Call.',
    whyStick: 'Cache-Warm-Up dauert Minuten. Hit-Ratio bei 40% statt 96% hält Upstream-Last hoch, was wiederum Rate-Limiting provoziert.',
    recovery: 'Stale-while-revalidate Pattern. Cache-Prewarming nach Incidents. TTL-Jitter gegen synchrone Expiration.',
    severity: 'hoch',
  ...
  },
  {
    ...
    mechanism: 'Memory-Pressure löst häufigere GC-Zyklen aus. GC verbraucht CPU, verlangsamt Request-Verarbeitung, Objekte akkumulieren.',
    whyStick: 'Heap-Fragmentierung persistiert. Full-GC-Zyklen bleiben häufig bis Compaction abgeschlossen. Stop-the-World kompoundiert mit CFS-Throttling.',
    recovery: 'Oft nur Pod-Restart (Heap-Reset). -XX:+UseG1GC mit -XX:MaxGCPauseMillis. -XX:MaxRAMPercentage=75.',
    severity: 'kritisch',
  ...
  },
  {
    ...
    mechanism: 'Hohe Allocation-Rate erzeugt kurzlebige Objekte in der Young Gen. Survivor-Spaces laufen über → Fragmentierung.',
    whyStick: 'Old-Gen-Fragmentierung bleibt nach Last-Reduktion. G1 muss viele Mixed-GC-Zyklen fahren um zu compacten.',
    recovery: 'G1 statt CMS. -XX:G1HeapRegionSize tunen. Allocation-Rate reduzieren (Object Pooling für heiße Pfade).',
    severity: 'mittel',
  ...
  },
  {
    ...
    mechanism: 'Alle Connections belegt. Threads blockieren auf getConnection() bis connectionTimeout (Default 30s).',
    whyStick: 'Drain-Time = Backlog / (Kapazität − Ankunftsrate). Connections durch Timeouts beschädigt, müssen neu aufgebaut werden.',
    recovery: 'connectionTimeout auf 10s (Fail-Fast). leakDetectionThreshold=60000. hikaricp_connections_pending monitoren.',
    severity: 'kritisch',
  ...
  },
  {
    ...
    mechanism: 'Circuit Breaker trippt bei Fehlerrate. Provider erholt sich, aber Breaker ist OPEN: keine Requests.',
    whyStick: 'Half-Open lässt nur 1–3 Probe-Requests durch. Slow Recovery by Design (Resilience4j waitDurationInOpenState).',
    recovery: 'waitDurationInOpenState tunen. permittedNumberOfCallsInHalfOpenState erhöhen. Fallback mit Stale-Daten.',
    severity: 'hoch',
  ...
  },
  {
    ...
    mechanism: 'Redis erreicht maxmemory, Eviction-Policy verdrängt Keys. Hit-Ratio sinkt, mehr Requests an Upstream-APIs.',
    whyStick: 'Evicted Keys müssen alle neu geladen werden. Upstream-Last bleibt hoch bis Cache warm. Kann Rate-Limiting triggern.',
    recovery: 'maxmemory erhöhen oder Redis-Cluster skalieren. TTL-Strategie reviewen. Separate Key-Namespaces pro Produktsparte.',
    severity: 'hoch',
  ...
  },
  {
    ...
    mechanism: '200 Threads busy + Accept-Queue (100) voll = Connection Refused. Oft Folgeproblem: langsame Upstream-Calls.',
    whyStick: 'Accept-Queue muss erst leerlaufen. Blockierte Threads halten Connections offen.',
    recovery: 'Root Cause finden (DB-Pool? Upstream-Timeout?). Timeouts verkürzen (<5s). Virtual Threads evaluieren.',
    severity: 'kritisch',
  ...
  },
  {
    ...
    mechanism: 'Container verbraucht CPU-Quota in der 100ms CFS-Periode. Wird pausiert bis zur nächsten Periode.',
    whyStick: 'Throttling erzeugt Request-Backlog. Backlog erhöht CPU-Bedarf. Erst wenn Backlog leer UND Last unter Quota.',
    recovery: 'CPU-Limit erhöhen. Burstable QoS (kein Limit, nur Request). GC-Tuning: -XX:MaxGCPauseMillis=100.',
    severity: 'hoch',
  ...
  },
  {
    ...
    mechanism: 'Working-Set überschreitet Memory-Limit. OOM-Kill → Pod restartet mit kaltem Cache, kaltem JIT.',
    whyStick: 'Restart = Cold Start. JIT muss neu kompilieren, Caches leer, Connections müssen aufgebaut werden.',
    recovery: '-XX:MaxRAMPercentage=75 (25% für Off-Heap). Memory-Limit = Heap + 30%. HeapDumpOnOutOfMemoryError.',
    severity: 'kritisch',
  ...
  },
  {
    ...
    mechanism: 'Bei Rescheduling (Node-Drain, Preemption) landen Pods ungleichmäßig. Hotspots entstehen.',
    whyStick: 'Kubernetes rescheduled laufende Pods nicht proaktiv (kein Rebalancing). Descheduler muss explizit konfiguriert werden.',
    recovery: 'Descheduler mit RemoveDuplicates/LowNodeUtilization. Pod Topology Spread Constraints. Resource Requests korrekt setzen.',
    severity: 'mittel',
  ...
  },
  {
    ...
    mechanism: 'HPA skaliert hoch bei Threshold. Neue Pods nehmen Last auf, Metrik sinkt, HPA skaliert runter.',
    whyStick: 'Cooldown-Period verzögert Scale-Down. Neue Pods starten kalt (JIT, Cache), brauchen mehr Ressourcen.',
    recovery: 'stabilizationWindowSeconds erhöhen (5–10 min). behavior.scaleDown.policies: max 1 Pod pro 60s.',
    severity: 'mittel',
  ...
  },
  {
    ...
    mechanism: 'Paketverluste triggern TCP Congestion Control. Window wird auf 1 MSS reduziert. Throughput bricht ein.',
    whyStick: 'Slow-Start braucht viele RTTs um das Window wieder hochzufahren. Bei hohem BDP: Recovery dauert Sekunden.',
    recovery: 'TCP BBR statt Cubic. Keep-Alive Connections (HTTP/2, Connection-Pooling). Retransmit-Timeouts tunen.',
    severity: 'mittel',
  ...
  },
  {
    ...
    mechanism: 'Rolling Updates ändern Endpoints. DNS-TTL und Client-Caches (JVM: unendlich!) halten stale Einträge.',
    whyStick: 'JVM cached DNS per Default unendlich. Nach Deployment zeigen Requests auf terminierte Pods → Connection Refused.',
    recovery: 'JVM: -Dnetworkaddress.cache.ttl=10. CoreDNS TTL anpassen. Headless Services für client-side LB.',
    severity: 'mittel',
  ...
  },
  {
    ...
    mechanism: 'Cloud-PVs haben IOPS-Limits. Überschreitung führt zu I/O-Queuing. MySQL/InnoDB Buffer Pool Flush: I/O-Spikes.',
    whyStick: 'I/O-Queue muss erst abgearbeitet werden. InnoDB-Dirty-Pages aufgestaut, Flush dauert.',
    recovery: 'PV-Typ upgraden (gp3 mit Provisioned IOPS). InnoDB: innodb_io_capacity tunen.',
    severity: 'hoch',
...
function toX(v) { return pad.l + v * cw }
function toY(v) { return pad.t + ch - v * ch }
function mkPath(pts) { return pts.map((p, i) => `${i === 0 ? 'M' : 'L'}${toX(p.x).toFixed(1)},${toY(p.y).toFixed(1)}`).join(' ') }
...
function downPathD(h) { return mkPath(buildPath(h.downPath)) }
  ...
  <div class="hysterese-catalog">
    ...
    <div class="controls">
      <div class="filter-buttons">
        ...
        >
          {{ cat.label }}
          <span v-if="cat.id !== 'all'" class="cat-count">({{ catCount(cat.id) }})</span>
        </button>
      </div>
    ...
    <div class="legend">
      ...
      <div class="legend-item">
        <div class="legend-box" :style="{ background: C.yellow + '10', borderColor: C.yellow + '20' }" />
    ...
    <!-- Cards -->
    <div class="cards">
      <div v-for="h in filtered" :key="h.id" class="card" :style="{ borderColor: expanded === h.id ? h.color + '50' : C.border }">
        <button class="card-header" @click="toggle(h.id)">
          <!-- Phase space mini -->
          <div class="phase-space">
            <svg :viewBox="`0 0 ${SVG_W} ${SVG_H}`">
              <path :d="areaPath(h)" :fill="h.color" opacity="0.06" />
              <path :d="upPathD(h)" fill="none" :stroke="C.red" stroke-width="1" opacity="0.15" stroke-dasharray="2,2" />
              <path :d="downPathD(h)" fill="none" :stroke="C.blue" stroke-width="1" opacity="0.15" stroke-dasharray="2,2" />
              <line :x1="pad.l" :y1="pad.t + ch" :x2="pad.l + cw" :y2="pad.t + ch" :stroke="C.dim" stroke-width="0.5" />
          ...
          <div class="card-text">
            <div class="card-title-row">
              ...
              <span class="severity-badge" :style="{ background: sevColor(h.severity) + '15', color: sevColor(h.severity), borderColor: sevColor(h.severity) + '30' }">{{ h.severity.toUpperCase() }}</span>
              <span class="category-badge" :style="{ background: catColor(h.category) + '12', color: catColor(h.category) }">{{ h.category }}</span>
            </div>
            <div class="card-mechanism">{{ h.mechanism }}</div>
          </div>
          <!-- Expand -->
          <span class="expand-arrow" :class="{ rotated: expanded === h.id }">▾</span>
        </button>
    ...
    <div class="summary">
      ...
      <div class="summary-counts">
        <div v-for="sev in ['kritisch', 'hoch', 'mittel']" :key="sev" class="summary-item">
          <span class="summary-number" :style="{ color: sevColor(sev) }">{{ HYSTERESES.filter(h => h.severity === sev).length }}</span>
